...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        # print cpts
        for joinFactor in eliminationOrder:
            cpts, joinedFactor = joinFactorsByVariable(cpts, joinFactor)
            # print cpts
            # print joinedFactor
            if len(joinedFactor.unconditionedVariables()) > 1:
                tmp = eliminate(joinedFactor, joinFactor)
                cpts.append(tmp)
        singleFactor = joinFactors(cpts)
        return normalize(singleFactor)
